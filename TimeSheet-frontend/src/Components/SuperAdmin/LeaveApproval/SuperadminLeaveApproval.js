...
function SuperadminLeaveApproval() {
  ...
  async function getLeaveData() {
    try {
      const response = await axios.get("http://172.31.224.1:4555/superadmin");
  ...
  async function approveSaveConfirmation() {
    ...
    try {
        ...
        const updatePromises = approvedLeavesRequest.map(async (admin) => {
            ...
            const response = await axios.put(`http://172.31.224.1:4555/superadmin/${admin.id}/approve`);
  ...
  async function rejectSaveConfirmation() {
    ...
    try {
      ...
      const updates = rejectLeaves.map(async (leave) => {
        ...
        const response = await axios.put(`http://172.31.224.1:4555/superadmin/${leave.id}/reject`,{
