...
function SearchAdmin() {
  ...
  async function getDataFromApi() {
    try {
      let response = await axios.get("http://13.233.98.111:4555/admins/getadmins"); //getadmin backend url
                ...
                <tbody>
                  {filteredAdminList.length > 0 && searchQuery
                    ? filteredAdminList.map((d) => (
                        <tr
                          key={d.adminId}
                          className={`text-center adminList-column ${
                            activeRow === d.adminId ? "table-active" : ""
                          }`}
