...
function ApproveTimesheet() {
  ...
  // console.log(timesheetDatas)
   ...
   async function getTimesheet() {
    try {
        const timesheets = await axios.get("http://172.31.224.1:4555/admins/all/new");
  ...
  async function approveSaveConfirmation() {
    ...
    try {
      ...
      const updates = approvedSheets.map(async (sheet) => {
        ...
        const response = await axios.put(`http://172.31.224.1:4555/admins/working-hours/${sheet.adminId}/approve-range?startDate=${sheet.startDate}&endDate=${sheet.endDate}`);
  ...
  async function rejectSaveConfirmation() {
    ...
    try {
      ...
      const updates = rejectSheets.map(async (sheet) => {
        ...
        // Make a PUT request to update the status of the sheet in the API
        const response = await axios.put(`http://172.31.224.1:4555/admins/working-hours/${sheet.adminId}/reject-range?startDate=${sheet.startDate}&endDate=${sheet.endDate}&reason=${rejectReason}`);
  ...
  function goEditPage(id,check,startDate,endDate) {
    ...
    if (check) {
      console.log(id)
      navigate('/superadmin/timesheetapproval/modifytimesheet/' + id,{state:{startDate,endDate}})
    } else {
